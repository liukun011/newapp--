    ...
    return (
      <div className="flex flex-col items-center justify-center min-h-screen bg-[#FFFFFF] relative">
        ...
        <div className="absolute top-0 left-0 right-0 p-4 flex items-center">
            <button onClick={onBack} className="p-2 -ml-2"><ArrowLeft size={24} className="text-[#0F2848]"/></button>
            <div className="flex-1 flex justify-center mr-8">
               <span className="font-medium text-lg text-[#0F2848]">被访企业名称</span>
               <div className="ml-1 text-[#8AA2BF]">✎</div>
            </div>
        </div>
        ...
        <p className="text-[#0F2848] font-medium tracking-wide">正在分析，请耐心等待</p>
      </div>
    );
  }

  return (
    <div className="flex flex-col h-screen bg-[#FFFFFF] relative">
      {/* Navbar */}
      <div className="flex items-center justify-between px-4 py-3 border-b border-[#E2EBF5]/60 bg-[#FFFFFF] z-10">
        <button onClick={onBack} className="p-2 -ml-2"><ArrowLeft size={24} className="text-[#0F2848]"/></button>
        <h1 className="text-lg font-medium text-[#0F2848]">自动生成问题集合</h1>
             ...
             >
                <div 
                  className={`mt-0.5 w-5 h-5 rounded-[4px] flex items-center justify-center flex-shrink-0 transition-colors duration-200 ${
                    isSelected ? 'bg-primary border-primary' : 'border-2 border-gray-300 bg-[#FFFFFF]'
                  }`}
                >
                   {isSelected && <Check size={14} className="text-white" strokeWidth={3} />}
                </div>
                <div className="text-[15px] text-[#0F2848] leading-relaxed font-normal">
                   {q.id}.{q.text}
                </div>
             </div>
           )
      ...
      {/* Bottom Bar */}
      <div className="px-6 py-3 border-t border-[#E2EBF5]/60 flex gap-4 bg-[#FFFFFF] z-10 shadow-[0_-4px_10px_rgba(0,0,0,0.03)]">
         <Button 
            variant="secondary" 
            block 
            onClick={onBack} 
            className="!rounded-[999px] !border-[#E2EBF5] !text-slate-600 !h-12 !text-[16px]"
         >
            取消
         </Button>
         <Button 
      ...
      {showModal && (
        <div className="absolute inset-0 z-50 flex items-center justify-center bg-black/40 backdrop-blur-[2px] transition-opacity duration-300">
           <div className="bg-[#FFFFFF] rounded-[18px] w-[80%] max-w-[320px] overflow-hidden shadow-[0_18px_44px_rgba(15,40,72,0.16)] animate-[scale_0.2s_ease-out]">
                 ...
                 <div className="space-y-5 mb-4">
                    {['招商', '信贷', '投资'].map(opt => (
                       <label key={opt} className="flex items-center justify-between cursor-pointer group" onClick={() => setStorageCategory(opt)}>
                          <span className="text-[15px] text-[#476285] font-medium">{opt}</span>
              ...
              <div className="grid grid-cols-2 gap-3 px-6 pb-6 mt-2">
                 <Button 
                    variant="secondary"
                    className="!rounded-[999px] !h-10 !text-[15px] border-[#E2EBF5]"
