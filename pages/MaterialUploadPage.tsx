...
const MaterialUploadPage: React.FC<MaterialUploadPageProps> = ({
  ...
  const handleAddQuestionConfirmThrottled = useThrottleFn(async () => {
      if (newQuestionName.trim()) {
        const newQuestion: QuestionInfo = {
          // id will be generated by backend, or use temp id locally if no backend sync yet
          // But here we rely on backend update to persist? No, dealService.createOrUpdateDealInst
          // doesn't return new ID list easily unless we refetch.
          // For now let's hope backend handles it or we re-fetch.
          // Actually we just send the list.
  ...
  const handleStartInterviewThrottled = useThrottleFn(onStartInterview, 1000);
  const handleConfirmThrottled = useThrottleFn(() => (onConfirm || onBack)(), 1000);
  ...
  const handleGenerateAIThrottled = useThrottleFn(onGenerateAI, 1000);
