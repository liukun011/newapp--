...
import { Search, Settings, FileText, Mic, Plus, LayoutTemplate, Trash2 } from 'lucide-react';
...
const HomePage: React.FC<HomePageProps> = ({ onNavigateToDetail }) => {
  ...
  const fetchDeals = async () => {
    ...
    try {
      const res = await dealService.queryDealInstList({
        pageNo: 0,
        pageSize: 50,
        dealInstTitle: searchTerm,
        status: activeTab === 'ongoing' ? ['1'] : ['2'], // 状态现在是字符串数组
      });
    ...
    <div className="flex flex-col h-screen relative bg-[#F7F8FA]">
      ...
      <div className="relative z-10 px-4 pt-4 pb-2">
        ...
        <div className="flex items-center gap-3 mb-4">
          <div className="flex-1">
            <Input 
              ...
              variant="outlined"
              containerClass="!py-2.5 !bg-white/80 backdrop-blur-sm border-transparent shadow-sm"
              style={{ fontSize: '14px' }}
            />
          </div>
          <button className="p-2 bg-white/80 rounded-full shadow-sm text-slate-600 hover:text-indigo-600">
        ...
        <div className="bg-white rounded-2xl p-4 shadow-sm flex items-center justify-between relative overflow-hidden">
          <div className="relative z-10">
            <h2 className="text-lg font-bold text-slate-800 mb-1">你好，我是你的访谈助手，小狸！</h2>
            <p className="text-xs text-slate-500">
              今日已有 {deals.filter(d => d.status === '1').length} 个进行中的访谈任务
            </p>
          ...
          <div className="w-24 h-24 -mr-2 -my-4 relative">
             <Mascot size="small" />
             <div className="absolute inset-0 z-20" /> {/* Transparent overlay to prevent image interaction */}
          </div>
      ...
      <div className="flex-1 flex flex-col relative z-10 mt-2 min-h-0">
        ...
        <div className="flex px-4 border-b border-gray-100 bg-transparent flex-shrink-0">
          <button 
            className={`mr-6 py-3 text-[15px] font-bold relative transition-colors ${activeTab === 'ongoing' ? 'text-slate-800' : 'text-gray-400'}`}
          ...
          <button 
            className={`py-3 text-[15px] font-bold relative transition-colors ${activeTab === 'archived' ? 'text-slate-800' : 'text-gray-400'}`}
            ...
            <>
              {deals.map((item) => (
                ...
                >
                  ...
                  >
                    {/* Icon Placeholder */}
                    <div className="w-12 h-12 rounded-xl flex items-center justify-center shadow-inner bg-indigo-50 text-indigo-500">
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <div className="flex items-center text-xs text-gray-400">
                        ...
                        <span>状态: {item.status === '1' ? '进行中' : '已归档'}</span>
                      </div>
                    </div>
